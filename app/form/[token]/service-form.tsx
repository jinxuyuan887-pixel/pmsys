...
export default function ExternalServiceForm({token}:{token:string}) {
  ...
  const isConsultation=consultationServices.has(meta.serviceName);
  return <main className="external-page"><section className="external-card"><div className="external-brand">♥ <strong>EAP 服务记录</strong></div><h1>提交服务执行记录</h1><p className="external-tip">已绑定：{meta.projectName} · {meta.serviceName}。无需选择项目，提交后自动归集。</p>
    <form action={submit}><div className="form-grid"><label>服务内容<input value={meta.serviceName} disabled/></label><label>服务人员<input name="provider" required/></label>
      <label>服务开始日期<input name="startDate" type="date" required/></label><label>服务结束日期<input name="endDate" type="date" required/></label><label>实际服务数量（{meta.unit}）<input name="quantity" type="number" min="1" defaultValue="1" required/></label>
      {isConsultation?<><label>咨询方式<select name="method" required defaultValue={meta.serviceName}><option>线上咨询</option><option>线下咨询</option><option>驻场咨询</option></select></label><label>咨询时长（分钟）<input name="duration" type="number" min="1" required/></label><label className="full">咨询概括<textarea name="summary" required/></label><label className="full">风险情况<select name="risk" required><option>无风险</option><option>需要跟进</option><option>重点关注</option></select></label></>:
